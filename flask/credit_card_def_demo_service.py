...
class Database:
	...
	df.to_sql(con=engine,name='credit_card_defaults_dataset',if_exists='append')
...
def train_dataset():
	...
	print(df)		
	sqldb = Database()
	df.to_sql(con=engine,name='credit_card_defaults_dataset',if_exists='replace',index=False) 
	sqldb.commit()
